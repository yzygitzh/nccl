...
num_warmups="${NPKIT_NCCL_NUM_WARMUPS:-0}"

# Number of nccl-tests iterations.
num_iters="${NPKIT_NCCL_NUM_ITERS:-10}"

# NPKit pre-definitions.
npkit_flags="${NPKIT_FLAGS}"
...
parallel-ssh -t 0 -h ${hostfile} "cd ${npkit_run_src_dir}/samples/npkit; python3 npkit_post_process.py --npkit_dump_dir=${npkit_dump_dir} --npkit_event_header_path=${npkit_run_src_dir}/src/include/npkit/npkit_event.h --output_dir=${npkit_post_process_dir}; cd ${npkit_post_process_dir}; tar cvzf npkit_result.tar.gz npkit_event_trace.json"
